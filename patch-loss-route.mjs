...
const lossRoute = `
  ...
  router.post('/:id/loss', async (req, res) => {
    try {
      ...
      // 1. Fechar a comanda sem pagamentos (O estoque já foi baixado quando o item foi adicionado)
...
if (!f.includes('/loss')) {
  f = f.replace("export function createOrdersRouter() {", "export function createOrdersRouter() {\n" + lossRoute);
