...
export const NfReceivingView: React.FC<NfReceivingViewProps> = ({ onBack, onImportarXml }) => {
  ...
  const [syncingChave, setSyncingChave] = useState<string | null>(null);

  const handleSyncNota = async (chaveNota: string) => {
    try {
      setSyncingChave(chaveNota);
      const res = await fetch(`${api.getApiUrl()}/fiscal/notas-recebidas/${chaveNota}/sync`, {
        method: 'POST'
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Erro ao sincronizar');

      if (data.xmlDisponivel) {
        alert(data.message || 'XML sincronizado com sucesso!');
      } else {
        alert(data.message || 'A SEFAZ ainda não liberou o arquivo XML desta nota.');
      }
      await loadNotas();
    } catch (err: any) {
      alert('Erro ao buscar na SEFAZ: ' + (err.message || err));
    } finally {
      setSyncingChave(null);
    }
  };
            ...
            {notas.map((nota) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {nota.status === 'ciencia_registrada' && (
                    <button
                      onClick={() => handleSyncNota(nota.chave)}
                      disabled={syncingChave === nota.chave}
                      title="Consultar se a SEFAZ já liberou o XML desta nota"
                      className="p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl transition cursor-pointer border border-slate-300 dark:border-slate-700 flex items-center gap-1.5 text-xs font-semibold"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${syncingChave === nota.chave ? 'animate-spin text-amber-500' : ''}`} />
                      {syncingChave === nota.chave ? 'Consultando...' : 'Buscar XML'}
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 font-bold text-sm rounded-xl transition cursor-pointer border ${
                      nota.status === 'importada'
