...
import { ArrowLeft, Printer, Search, FileText, Loader2, RefreshCw, CheckCircle, Package } from 'lucide-react';
...
export const NfceReprintView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleConsultByNumber = async (numToSearch?: string) => {
    ...
    try {
      const res = await fetch(`${api.getApiUrl()}/fiscal/reprint-by-number/${encodeURIComponent(num)}`);
      const data = await res.json();
    ...
    <div className="space-y-6 max-w-4xl mx-auto pt-4 pb-20">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h2 className="text-xl font-black text-white">Reimprimir NFC-e</h2>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex gap-3">
              <input 
                ...
                className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:border-sky-500 outline-none transition font-mono text-sm"
              />
              <button 
          ...
          {result && result.success && (
            <div className="p-6 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl space-y-4 animate-in fade-in slide-in-from-bottom-2">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  <FileText className="w-6 h-6 text-emerald-400 mt-0.5" />
                  <div>
                    <h3 className="text-base font-black text-emerald-400">
                      NFC-e Nº {result.nota?.numero || numeroNota} Encontrada!
                    </h3>
                    <p className="text-xs text-slate-300 mt-1">
                      Série: {result.nota?.serie || '1'} • Status: <strong className="uppercase text-emerald-400">{result.status || 'Autorizado'}</strong>
                    </p>
                    {result.chaveAcesso && (
                      <p className="text-[11px] text-slate-400 mt-1 break-all font-mono">
        ...
        {loadingRecent ? (
          ...
          <div className="divide-y divide-slate-800">
            {recentNotas.map(nota => (
              <div key={nota.id} className="p-4 flex items-center justify-between gap-4 hover:bg-slate-850/50 transition">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3.5 py-2 bg-sky-500/15 hover:bg-sky-500 text-sky-300 hover:text-white border border-sky-500/30 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer active:scale-95"
                  >
