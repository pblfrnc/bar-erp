  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pt-4 pb-20">
      <div className="flex items-center justify-between mb-4">
        <button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-white transition cursor-pointer">
          <ArrowLeft className="w-4 h-4" /> Voltar ao Painel Fiscal
        </button>

        <button
          onClick={loadCancelableNotes}
          className="p-2 text-slate-400 hover:text-white rounded-xl bg-slate-800 hover:bg-slate-700 transition cursor-pointer flex items-center gap-1.5 text-xs font-bold"
        >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-white">Cancelamento de NFC-e</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Conforme regras da SEFAZ, o cancelamento é permitido exclusivamente em até <strong className="text-white">30 minutos</strong> após a emissão.
            </p>
          </div>
        </div>

        <div className="mt-4 p-3 bg-slate-950/70 border border-slate-800 rounded-xl text-[11px] text-slate-400 flex items-start gap-2">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center text-slate-500 text-xs flex flex-col items-center gap-2">
            ...
            <span className="font-bold text-slate-400">Nenhuma nota emitida nos últimos 30 minutos.</span>
            <span>Todas as notas anteriores já tiveram seu prazo legal de cancelamento expirado.</span>
          </div>
        ) : (
          <div className="divide-y divide-slate-800">
            {notas.map(nota => (
              <div key={nota.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-850/50 transition">
                <div className="space-y-1">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-amber-500/15 text-amber-400 border border-amber-500/30 flex items-center gap-1">
                ...
                <button
                  ...
                  className="px-4 py-2.5 bg-rose-500/15 hover:bg-rose-500 text-rose-300 hover:text-white border border-rose-500/30 rounded-xl text-xs font-black uppercase tracking-wider transition flex items-center justify-center gap-1.5 cursor-pointer active:scale-95 self-start sm:self-auto"
                >
      ...
      {selectedNota && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-slate-900 border border-rose-500/40 rounded-3xl w-full max-w-lg p-6 sm:p-8 shadow-2xl space-y-5">
            ...
            <div className="flex items-center gap-3 text-rose-400">
              ...
              <div>
                <h3 className="text-lg font-black text-white">Dupla Confirmação de Cancelamento</h3>
                <span className="text-xs text-rose-400 font-bold">Ação Irreversível perante a SEFAZ</span>
              </div>
            </div>
            ...
            <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-2 text-xs">
              <div className="flex justify-between text-slate-400">
                ...
                <span className="font-mono font-bold text-white">#{selectedNota.numero} (Série {selectedNota.serie})</span>
              </div>
              <div className="flex justify-between text-slate-400">
                <span>Valor Total:</span>
                <span className="font-mono font-bold text-emerald-400">R$ {(selectedNota.valorTotal || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-slate-400">
                <span>Prazo Restante:</span>
                <span className="font-bold text-amber-400">~{selectedNota.minutesRemaining} minutos</span>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                autoFocus
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white text-xs focus:border-rose-500 outline-none transition resize-none"
              />
              <div className="flex justify-between items-center mt-1 text-[11px]">
                <span className={justificativa.length >= 15 ? 'text-emerald-400 font-bold' : 'text-rose-400'}>
            ...
            <div className="pt-2 flex gap-3">
              <button
                ...
                className="flex-1 py-3 px-4 rounded-xl font-bold text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 transition cursor-pointer"
              >
                Voltar / Não Cancelar
              </button>
