...
export const OrderHistoryModal: React.FC<OrderHistoryModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 dark:bg-black/80 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-6xl h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/70 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">Comandas Finalizadas & Histórico</h2>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Consulte itens consumidos, descontos, 10% de serviço e 2ª via de comprovante</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 transition cursor-pointer"
            >
        ...
        {/* Filtros */}
        <div className="px-6 py-3 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900 flex flex-col sm:flex-row items-center gap-3 justify-between">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:border-amber-500 focus:outline-none transition"
            />
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <input
              ...
              className="bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white px-3 py-2 focus:border-amber-500 focus:outline-none"
            />
            {selectedDate && (
              <button
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto border-r border-slate-200 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-950/40 scrollbar-thin">
            {loading ? (
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {isLoss ? (
                            <span className="px-1.5 py-0.5 rounded text-[9px] font-black uppercase bg-rose-500/15 text-rose-600 dark:text-rose-300 border border-rose-500/30">
                              Calote / Perda
                            </span>
                          ) : (
          ...
          <div className="w-full md:w-[460px] bg-white dark:bg-slate-900 flex flex-col justify-between overflow-y-auto scrollbar-thin border-t md:border-t-0 border-slate-200 dark:border-slate-800">
            {selectedOrder ? (
              <div className="p-6 space-y-5 flex-1">
                
                {/* Header do Card de Detalhes */}
                <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-xs">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-200 dark:border-slate-800/80">
                    <div>
                      <span className="text-slate-400 dark:text-slate-500 block text-[10px]">Data & Hora</span>
                      <span className="text-slate-700 dark:text-slate-300 font-medium">
                        {selectedOrder.closedAt ? new Date(selectedOrder.closedAt).toLocaleString('pt-BR') : '—'}
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-400 dark:text-slate-500 block text-[10px]">Atendente</span>
                      <span className="text-slate-700 dark:text-slate-300 font-medium">
                  ...
                  {selectedOrder.customerName && (
                    <div className="text-xs pt-2 border-t border-slate-200 dark:border-slate-800/80">
                      <span className="text-slate-400 dark:text-slate-500 block text-[10px]">Cliente Identificado</span>
                      <span className="text-slate-700 dark:text-slate-300 font-medium">{selectedOrder.customerName}</span>
                    </div>
                  )}
                </div>
                ...
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-slate-400 dark:text-slate-500">Qtd / Unit / Total</span>
                  </h4>

                  <div className="bg-slate-50 dark:bg-slate-950/60 rounded-2xl border border-slate-200 dark:border-slate-800 divide-y divide-slate-200 dark:divide-slate-800/50 overflow-hidden shadow-xs">
                    {selectedOrder.items?.map((item, idx) => (
                      <div key={idx} className="p-3 flex items-center justify-between text-xs">
                        <div className="flex-1 min-w-0 pr-2">
                          ...
                          {item.notes && (
                            <div className="text-[10px] text-amber-600 dark:text-amber-400/80 italic">
                              Obs: {item.notes}
                            </div>
                          )}
                ...
                <div className="bg-slate-50 dark:bg-slate-950/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-2 text-xs shadow-xs">
                  <div className="flex justify-between text-slate-500 dark:text-slate-400">
                    <span>Subtotal dos Itens:</span>
                    <span className="font-mono text-slate-800 dark:text-slate-200">R$ {(selectedOrder.subtotal || 0).toFixed(2)}</span>
                  </div>
                  ...
                  <div className="flex justify-between pt-2 border-t border-slate-200 dark:border-slate-800 text-sm font-black">
                    <span className="text-slate-900 dark:text-white uppercase">Total Final:</span>
                  ...
                  {selectedOrder.payments && selectedOrder.payments.length > 0 && (
                    <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-1">
                      ...
                      {selectedOrder.payments.map((p, pIdx) => (
                        <div key={pIdx} className="flex justify-between text-[11px] text-slate-500 dark:text-slate-400">
                          <span>• {p.method}:</span>
                ...
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl text-xs font-black uppercase tracking-wider bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition flex items-center justify-center gap-2 cursor-pointer shadow-xs active:scale-95"
                >
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 text-xs p-8 text-center">
