      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/90">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-slate-900 dark:text-white font-bold text-base">Consulta Oficial de NCM</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">Classificação fiscal e alíquotas oficiais da Receita Federal</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-4">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white font-mono text-sm focus:border-purple-500 outline-none placeholder-slate-400 dark:placeholder-slate-500"
              />
            </div>
            <button
              type="submit"
          ...
          {/* Atalhos Rápidos */}
          <div>
            <p className="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 tracking-wider mb-2">Atalhos Comuns no Bar & Restaurante</p>
            <div className="flex flex-wrap gap-1.5">
              {quickCodes.map((item, idx) => (
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800/80 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-[11px] font-medium transition cursor-pointer border border-slate-200 dark:border-slate-700/60"
                >
                  <span className="font-mono text-purple-600 dark:text-purple-400 font-bold">{item.code}</span> — {item.label}
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Resultado */}
          {error && (
            <div className="bg-rose-500/10 border border-rose-500/30 rounded-2xl p-4 text-xs text-rose-600 dark:text-rose-300 flex items-start gap-2.5">
            ...
            <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-base font-black text-slate-900 dark:text-white">{result.codigo}</span>
                </div>
                <span className="px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 text-[10px] font-bold uppercase border border-emerald-500/30">
                  Válido na SEFAZ
                </span>
              </div>
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{result.descricao || 'Sem descrição cadastrada'}</p>
              </div>

              {(result.aliquotaNacional !== undefined || result.aliquotaEstadual !== undefined) && (
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <div className="bg-white dark:bg-slate-900 rounded-xl p-2.5 border border-slate-200 dark:border-slate-800/80 shadow-xs">
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-1">
                      <Percent className="w-3 h-3 text-purple-600 dark:text-purple-400" /> Imposto Federal
                    </p>
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-xl p-2.5 border border-slate-200 dark:border-slate-800/80 shadow-xs">
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-1">
                      <Percent className="w-3 h-3 text-purple-600 dark:text-purple-400" /> Imposto Estadual
                    </p>
                    <p className="text-sm font-mono font-bold text-slate-900 dark:text-white mt-0.5">
