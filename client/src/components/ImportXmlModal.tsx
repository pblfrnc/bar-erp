      ...
      <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-lg shadow-2xl">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-white font-bold">Importar do Sistema Antigo</h2>
              <p className="text-xs text-slate-400">XML de exportação de tabelas</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="bg-slate-800/50 rounded-xl p-3 text-xs text-slate-400 leading-relaxed">
            ...
            <span className="text-white font-medium">Categorias</span> e{' '}
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition
              ${file ? 'border-sky-500/50 bg-sky-500/5' : 'border-slate-700 hover:border-slate-500 bg-slate-800/30'}`}
          >
            <input
              ref={inputRef}
              type="file"
            ...
            {file ? (
              <div className="space-y-1">
                ...
                <p className="text-slate-400 text-xs">{(file.size / 1024).toFixed(1)} KB — clique para trocar</p>
              </div>
            ) : (
              <div className="space-y-2">
                <UploadCloud className="w-8 h-8 text-slate-500 mx-auto" />
                <p className="text-slate-400 text-sm">Arraste o arquivo XML aqui ou <span className="text-sky-400 underline">clique para selecionar</span></p>
              </div>
            )}
          </div>
              ...
              {result.ok ? (
                <>
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-emerald-300 font-bold text-sm">Importação concluída!</p>
                  </div>
                  {result.resumo && (
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-slate-900/60 rounded-xl p-2.5">
                        <p className="text-slate-400">Categorias importadas</p>
                        <p className="text-white font-black text-lg">{result.resumo.categorias.importadas}</p>
                      </div>
                      <div className="bg-slate-900/60 rounded-xl p-2.5">
                        <p className="text-slate-400">Fornecedores importados</p>
                        <p className="text-white font-black text-lg">{result.resumo.fornecedores.importados}</p>
                      ...
                      {result.resumo.produtos.importados + result.resumo.produtos.atualizados > 0 && (
                        <>
                          <div className="bg-slate-900/60 rounded-xl p-2.5">
                            <p className="text-slate-400">Produtos importados</p>
                            <p className="text-white font-black text-lg">{result.resumo.produtos.importados}</p>
                          </div>
                          <div className="bg-slate-900/60 rounded-xl p-2.5">
                  ...
                  {result.resumo && result.resumo.erros.length > 0 && (
                    <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3">
                      <p className="text-amber-300 text-xs font-bold mb-1">⚠️ {result.resumo.erros.length} erros ignorados:</p>
                      <ul className="text-amber-200/70 text-[10px] space-y-0.5 max-h-24 overflow-auto">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={onClose}
              className="px-5 py-3 rounded-2xl font-bold text-sm bg-slate-800 hover:bg-slate-700 text-slate-300 transition"
            >
